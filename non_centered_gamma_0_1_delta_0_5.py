...
def true_model(prior):
    ...
    prior.sample(noise,mtrue,add_mean=True)
    return mtrue
def no_noise(prior):
    """Return a noisy sample from the prior.
    """
...
def main(args):
    ...
    mean_expr=dl.Expression("1", element=Vh[PARAMETER].ufl_element())
    mean=dl.interpolate(mean_expr,Vh[PARAMETER])

    # Set up prior on parameter.
    prior = BiLaplacianPrior(Vh[PARAMETER], GAMMA, DELTA, mean=mean.vector() ,robin_bc=True)
    #mtrue = true_model(prior)  
    #mtrue = no_noise(prior)    
    mtrue=dl.Expression("1+sin(x[0])*sin(x[1])", element=Vh[PARAMETER].ufl_element())
    mtrue=dl.interpolate(mtrue,Vh[PARAMETER]).vector()
    ...
    if rank == 0:
        ...
        plt.yscale('log')
        plt.show()
        print("Rel error is", (posterior.mean-mtrue).norm('l2')/mtrue.norm('l2'))
        print("Posterior variance is", post_pw_variance.norm('l2')) 
        print("Prior variance is", pr_pw_variance.norm('l2'))
